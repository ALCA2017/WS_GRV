...
public class EstadosRepositoryImpl implements EstadosRepository {
    ...
    @Override
    public Estados buscar_rol_id(int idEstados) throws Exception {
        try {
            CriteriaBuilder cb = entityManager.getCriteriaBuilder();
            CriteriaQuery cq = cb.createQuery();
            Root<Estados> root = cq.from(Estados.class);
            
            cq.select(root).where(
                cb.equal(root.get(Estados_.idEstado), idEstados)
            );
            ...
            return obj != null ? (Estados) obj : null;
        } catch (Exception e) {
            LOGGER.error(e.getMessage());
            throw e;
        }
